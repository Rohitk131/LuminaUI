...
const TextRain = () => {
  const heading = "Welcome to the Future";
  const subheading = "Where Innovation Meets Design";
  ...
  useEffect(() => {
    ...
    const maxDelay = Math.max(...[...headingLetters, ...subheadingLetters].map(l => l.startDelay));
    setTimeout(() => {
        ...
        <div className="text-center">
          ...
          <div className="relative h-20 sm:h-24 md:h-32 mb-8 overflow-visible">
            <div className="flex justify-center flex-wrap">
              {letters.filter(l => l.isHeading).map((letter) => (
                <div
                  key={letter.id}
                  className="relative inline-block transform-gpu"
                  onMouseEnter={() => !isAnimating && setHoveredLetter(letter.id)}
                ...
                >
                  <span 
                    ...
                    style={{
                      transform: !isAnimating && getNeighborIndices(letter.id).some(n => n.id === hoveredLetter)
                        ? `translateY(${Math.sin(getNeighborIndices(letter.id).find(n => n.id === hoveredLetter).distance) * 8}px) 
                           scale(${1 + (1 / (getNeighborIndices(letter.id).find(n => n.id === hoveredLetter).distance + 1))})`
                        : 'translateY(0) scale(1)',
                      color: getNeighborIndices(letter.id).some(n => n.id === hoveredLetter)
                        ? `hsl(${210 + getNeighborIndices(letter.id).find(n => n.id === hoveredLetter).distance * 15}, 100%, 70%)`
                        : 'white',
                      textShadow: getNeighborIndices(letter.id).some(n => n.id === hoveredLetter)
              ...
              {letters.filter(l => !l.isHeading).map((letter) => (
                ...
                >
                  <span 
                    ...
                    style={{
                      transform: !isAnimating && getNeighborIndices(letter.id).some(n => n.id === hoveredLetter)
                        ? `translateY(${Math.sin(getNeighborIndices(letter.id).find(n => n.id === hoveredLetter).distance) * 4}px)
                           scale(${1 + (0.5 / (getNeighborIndices(letter.id).find(n => n.id === hoveredLetter).distance + 1))})`
                        : 'translateY(0) scale(1)',
                      color: getNeighborIndices(letter.id).some(n => n.id === hoveredLetter)
                        ? `hsl(${170 + getNeighborIndices(letter.id).find(n => n.id === hoveredLetter).distance * 20}, 100%, 80%)`
                        : 'rgb(148, 163, 184)',
                      textShadow: getNeighborIndices(letter.id).some(n => n.id === hoveredLetter)
